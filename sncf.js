...
Promise.all([fetchPlaces(from), fetchPlaces(to)]).then((data) => {
  const [dataFrom, dataTo] = data;
  const fromStopAreas = dataFrom.places.filter(
    (p) => p.embedded_type === "stop_area"
  );
  const toStopAreas = dataTo.places.filter(
    (p) => p.embedded_type === "stop_area"
  );
  ...
  fetch(url, {
    ...
    .then((data) => {
      ...
      console.log(
        "journeys: ",
        data.journeys.map((j) => ({
          departure: moment(j.departure_date_time, "YYYYMMDDTHHmm").format(
            "MMMM D, YYYY, HH:mm"
          ),
          arrival: moment(j.arrival_date_time, "YYYYMMDDTHHmm").format(
            "MMMM D, YYYY, HH:mm"
          ),
        }))
      );
